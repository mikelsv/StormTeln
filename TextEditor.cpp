...
{
	//RemoveLine(0, mLines.size());
	mLines.clear();
	mLines.push_back(Line());
	mTextChanged = true;
}

int TextEditor::InsertTextAt(Coordinates& /* inout */ aWhere, const char * aValue, const unsigned int aSize, PaletteIndex palette)
{
	//assert(!mReadOnly);

	int cindex = GetCharacterIndex(aWhere);
	const char *eValue = aValue + aSize;
	int totalLines = 0;
	...
	while (aValue < eValue)
	{
		...
		else
		{
			...
			while (d-- > 0 && *aValue != '\0')
				line.insert(line.begin() + cindex++, Glyph(*aValue++, palette));
			++aWhere.mColumn;
		}
...
{
	InsertText(aValue.c_str(), aValue.size());
}

void TextEditor::InsertText(const char * aValue, const unsigned int aSize)
{
	if (aValue == nullptr)
		return;
	...
	totalLines += InsertTextAt(pos, aValue, aSize);
...
void TextEditor::AppendText(const char *aValue, const unsigned int aSize, PaletteIndex palette)
{
	if (aValue == nullptr)
		return;

	auto lpos = GetActualCursorCoordinates();
	auto start = std::min(lpos, mState.mSelectionStart);
	int totalLines = lpos.mLine - start.mLine;

	Coordinates pos((int)mLines.size() - 1, mLines[mLines.size() - 1].size());
	//Advance(pos);

	totalLines += InsertTextAt(pos, aValue, aSize, palette);

	SetSelection(lpos, lpos);
	SetCursorPosition(lpos);
	//Colorize(start.mLine - 1, totalLines + 2);
}
...
void TextEditor::UndoRecord::Undo(TextEditor * aEditor)
{
	...
	if (!mRemoved.empty())
	{
		...
		aEditor->InsertTextAt(start, mRemoved.c_str(), mRemoved.size());
...
void TextEditor::UndoRecord::Redo(TextEditor * aEditor)
{
	...
	if (!mAdded.empty())
	{
		...
		aEditor->InsertTextAt(start, mAdded.c_str(), mAdded.size());
